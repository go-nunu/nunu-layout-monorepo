    ...
    <div class="relative flex-b mt-2.5 max-md:block max-md:mt-1">
      <div class="w-112 mr-5 max-md:w-full max-md:mr-0">
        <div class="art-card-sm relative p-9 pb-6 overflow-hidden text-center">
          ...
          <p class="mt-5 text-sm">{{ date }}，欢迎回来</p>
          ...
          <div class="w-75 mx-auto mt-7.5 text-left">
            ...
            <div class="mt-2.5">
              ...
              <span class="ml-2 text-sm">账号：{{ accountName }}</span>
            </div>
            <div class="mt-2.5">
              <ArtSvgIcon icon="ri:phone-line" class="text-g-700" />
              <span class="ml-2 text-sm">{{ userInfo.phone || '未设置手机号' }}</span>
            </div>
            <div class="mt-2.5">
...
<script setup lang="ts">
  ...
  const displayName = computed(
    () => userInfo.value.nickname || userInfo.value.userName || userInfo.value.username || '未命名用户'
  )
  const accountName = computed(() => userInfo.value.username || userInfo.value.userName || '-')
  ...
  watch(userInfo, syncUserInfoToForm, { deep: true })

  function syncUserInfoToForm() {
    form.username = userInfo.value.username || userInfo.value.userName || ''
    form.nickname = userInfo.value.nickname || userInfo.value.userName || ''
